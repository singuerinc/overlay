...
class RulerView extends React.Component<IRuler & IProps, IState> {
  ...
  public render() {
    ...
    return (
      <RulerWrapper ref={this.el} locked={locked}>
        <Coords x={x} y={y} />
  ...
  private split = (direction: SplitDirection) => () => {
    ...
    this.setState(resize(width, height), () => {
      ...
      this.props.split(direction, this.props.id, x, y, width, height, this.props.color);
    });
  };
}

const RulerWrapper = styled.div`
  pointer-events: ${(props: { locked: boolean }) => (props.locked ? 'none' : 'all')};
  cursor: ${(props: { locked: boolean }) => (props.locked ? 'none' : 'inherit')};
  position: fixed;
  display: block;
